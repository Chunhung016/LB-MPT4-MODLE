...
interface ChinesePartBExperienceProps {
  onExit: () => void;
}

function ChinesePartBContent({ onExit }: ChinesePartBExperienceProps) {
  const { currentScreen, setCurrentScreen } = useApp();

  return (
    <HoneycombBackground>
      <button
        id="chinese-part-b-exit"
        type="button"
        onClick={() => {
          sound.playPop();
          onExit();
...
export function ChinesePartBExperience({ onExit }: ChinesePartBExperienceProps) {
  return (
    <div className="fixed inset-0 z-[70] overflow-y-auto bg-[#FFFBEB]">
      <AppProvider>
        <ChinesePartBContent onExit={onExit} />
      </AppProvider>
    </div>
  );
}
