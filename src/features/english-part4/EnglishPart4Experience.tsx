...
interface EnglishPart4ExperienceProps {
  onExit: () => void;
}

function EnglishPart4Content({ onExit }: EnglishPart4ExperienceProps) {
  const { currentScreen, setCurrentScreen } = useApp();

  return (
    <HoneycombBackground>
      <button
        id="english-part4-exit"
        type="button"
        onClick={() => {
          sound.playPop();
          onExit();
...
export function EnglishPart4Experience({ onExit }: EnglishPart4ExperienceProps) {
  return (
    <div className="fixed inset-0 z-[70] overflow-y-auto bg-[#FFFBEB]">
      <AppProvider>
        <EnglishPart4Content onExit={onExit} />
      </AppProvider>
    </div>
  );
}
