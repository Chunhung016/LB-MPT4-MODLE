...
import { THEME_1_WORDS } from '../data/theme1Words';
...
export const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'general' | 'images' | 'stimulus' | 'starters' | 'vocab' | 'model' | 'voice' | 'audio' | 'data'
  >('images');
        ...
        >
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <h2 className="text-base sm:text-xl font-black text-[#78350F] uppercase tracking-tight">
                  SPELLING BEE • Content Settings
                </h2>
                <p className="text-[11px] sm:text-xs font-bold text-[#78350F]/80">
                  Configure Spelling Bee prompts, vocabulary and audio • Press [1] to Toggle
                </p>
              </div>
            </div>
          ...
          <div className="flex border-b-2 border-[#78350F] bg-[#FEF3C7] px-4 pt-2 gap-1.5 overflow-x-auto">
            ...
            >
              ...
              <span>Word Images & Links ({THEME_1_WORDS.length})</span>
            </button>
          ...
          <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-6">
            ...
            {/* WORD IMAGES & LINKS TAB */}
            {activeTab === 'images' && (
              <div className="space-y-5">
                {/* Global Image Rules Banner */}
                <div className="bg-[#FFFBEB] p-4 rounded-2xl border-2 border-[#FDE68A] space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <h3 className="text-sm font-black text-[#78350F] uppercase flex items-center gap-2">
                        <ImageIcon className="w-4 h-4 text-amber-600" />
                        <span>Visual Clues & Randomization Rule</span>
                      </h3>
                      <p className="text-xs text-[#B45309] font-medium mt-0.5">
                        • When image is shown: <strong>No 1st letter</strong> is given (full spelling test).
                        <br />
                        • When no image is shown: <strong>1st letter is provided</strong> in the 1st box.
                      </p>
                    </div>

                    <label className="flex items-center gap-2.5 bg-white px-3 py-2 rounded-xl border-2 border-[#78350F] cursor-pointer shadow-xs shrink-0">
                      <input
                        type="checkbox"
                        checked={settings.enableImageClues}
                        onChange={(e) => updateSettings({ enableImageClues: e.target.checked })}
                        className="w-4 h-4 accent-[#F59E0B]"
                      />
                      <span className="text-xs font-black text-[#78350F]">
                        {settings.enableImageClues ? 'Image Mode Active' : 'Image Mode Off'}
                      </span>
                    </label>
                  </div>

                  {/* Mode Probability Selector */}
                  <div className="pt-3 border-t border-[#FDE68A] flex flex-wrap items-center gap-3 justify-between">
                    <span className="text-xs font-black text-[#78350F] uppercase">
                      Image Appearance Rule:
                    </span>

                    <div className="flex items-center gap-1.5 flex-wrap">
                      <button
                        type="button"
                        onClick={() => updateSettings({ imageRandomRate: 0.5, enableImageClues: true })}
                        className={`px-3 py-1 rounded-xl text-xs font-black border-2 transition-all cursor-pointer ${
                          settings.enableImageClues && settings.imageRandomRate === 0.5
                            ? 'bg-[#F59E0B] text-[#78350F] border-[#78350F] shadow-xs'
                            : 'bg-white text-slate-600 border-slate-300 hover:border-[#78350F]'
                        }`}
                      >
                        🎲 50% Random (Image or 1st Letter)
                      </button>

                      <button
                        type="button"
                        onClick={() => updateSettings({ imageRandomRate: 1.0, enableImageClues: true })}
                        className={`px-3 py-1 rounded-xl text-xs font-black border-2 transition-all cursor-pointer ${
                          settings.enableImageClues && settings.imageRandomRate === 1.0
                            ? 'bg-[#F59E0B] text-[#78350F] border-[#78350F] shadow-xs'
                            : 'bg-white text-slate-600 border-slate-300 hover:border-[#78350F]'
                        }`}
                      >
                        🖼️ Always Show Image (No 1st Letter)
                      </button>

                      <button
                        type="button"
                        onClick={() => updateSettings({ enableImageClues: false })}
                        className={`px-3 py-1 rounded-xl text-xs font-black border-2 transition-all cursor-pointer ${
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                  <div className="relative flex-1">
                    <input
                      type="text"
                      placeholder="Search word by English or Chinese..."
                      value={imageSearch}
                      onChange={(e) => setImageSearch(e.target.value)}
                      className="w-full pl-3 pr-8 py-2 bg-[#FFFBEB] border-2 border-[#78350F] rounded-xl text-xs font-bold text-[#78350F] placeholder-[#78350F]/50"
                    />
                    {imageSearch && (
                      <button
                  ...
                  <button
                    onClick={() => {
                      updateSettings({ wordImages: {} });
                      sound.playPop();
                    }}
                    className="px-3 py-2 bg-amber-100 hover:bg-amber-200 text-[#78350F] font-black text-xs rounded-xl border-2 border-[#78350F] flex items-center justify-center gap-1.5 cursor-pointer shrink-0"
                    title="Reset all customized image links to default photos"
                  >
                    <RefreshCw className="w-3.5 h-3.5" />
                    <span>Reset All to Default Photos</span>
                  </button>
                </div>

                {/* Word List with Image Link Inputs */}
                <div className="space-y-3 max-h-[480px] overflow-y-auto pr-1">
                  {THEME_1_WORDS.filter(
                    (w) =>
                      w.word.toLowerCase().includes(imageSearch.toLowerCase()) ||
                      w.chinese.includes(imageSearch) ||
                      w.description.toLowerCase().includes(imageSearch.toLowerCase())
                  ).map((word) => {
                    const customImg = settings.wordImages?.[word.id];
                    const isCustomized = customImg !== undefined;
                    const displayVal = isCustomized ? customImg : word.imageUrl || '';

                    return (
                      <div
                        key={word.id}
                        className="p-3 bg-[#FFFBEB] rounded-2xl border-2 border-[#FDE68A] hover:border-[#F59E0B] transition-all flex flex-col sm:flex-row items-start sm:items-center gap-3.5"
                      >
                        {/* Image Thumbnail */}
                        <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-white border-2 border-[#78350F] overflow-hidden shrink-0 shadow-xs flex items-center justify-center p-1">
                          {displayVal ? (
                            <img
                              src={displayVal}
                              alt={word.word}
                            ...
                            <ImageIcon className="w-6 h-6 text-amber-700/40" />
                          )}
                        ...
                        <div className="flex-1 w-full space-y-1.5">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs font-bold text-amber-700 bg-amber-200/70 px-2 py-0.5 rounded-lg border border-amber-300">
                                {word.chinese}
                              </span>
                              <span className="text-[10px] font-bold text-slate-500">
                                ({word.boxCount} letters)
                              </span>
                            </div>
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              type="text"
                              value={displayVal}
                              placeholder="Paste picture URL here (or leave blank to remove)"
                              onChange={(e) => {
                                const newImages = { ...(settings.wordImages || {}) };
                                const val = e.target.value;
                                newImages[word.id] = val;
                                updateSettings({ wordImages: newImages });
                              }}
                              className="flex-1 px-3 py-1.5 bg-white border-2 border-[#78350F]/40 focus:border-[#78350F] rounded-xl text-xs font-medium text-[#78350F]"
                            />
                            ...
                            {displayVal && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="p-1.5 bg-white hover:bg-amber-100 text-[#78350F] rounded-xl border border-[#78350F]/40 cursor-pointer shrink-0"
                            ...
                            {isCustomized && (
                              <button
                                ...
                                className="px-2.5 py-1.5 bg-amber-100 hover:bg-amber-200 text-[#78350F] text-xs font-black rounded-xl border border-[#78350F] cursor-pointer shrink-0"
                                title="Reset this word to original default photo"
                              >
                                Reset to Default
                              </button>
                            )}
            ...
            {/* READ ALOUD VOICE TAB */}
            {activeTab === 'voice' && (
              <div className="space-y-4">
                <div>
                  <h3 className="text-sm font-black text-[#78350F] uppercase flex items-center gap-2">
                    <Volume2 className="w-4 h-4 text-amber-600" />
                    <span>Text-to-Speech (Read Aloud) Voice Settings</span>
                  </h3>
                  <p className="text-xs text-[#B45309] font-medium mt-0.5">
                    Customize voice accent, rate/speed, and pitch used for reading spelling words, phonics clues, and model answers aloud.
                  </p>
                </div>
            ...
            {activeTab === 'audio' && (
              <div className="space-y-6">
                ...
                {/* Quick Voice Settings within Audio Tab */}
                <div className="pt-4 border-t-2 border-[#FDE68A]">
                  <h3 className="text-sm font-black text-[#78350F] uppercase flex items-center gap-2 mb-3">
                    <Volume2 className="w-4 h-4 text-amber-600" />
                    <span>Quick Read Aloud Voice Settings</span>
                  </h3>
                  <VoiceSettingsPanel />
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-[#FFFBEB] p-4 rounded-2xl border-2 border-[#FDE68A] space-y-3">
                  ...
                  <p className="text-[11px] text-[#B45309] font-medium">
                    Save your custom Spelling Bee questions, stimulus notes, and vocabulary lists as a JSON backup.
                  </p>
                </div>
                ...
                <div className="pt-4 border-t-2 border-[#FEF3C7] flex items-center justify-between">
                  <div>
                    <span className="text-xs font-black text-red-600 block">
                      Reset to Default Spelling Bee
                    </span>
                    <span className="text-[10px] text-slate-500 font-medium">
                      Clears local edits and restores default Spelling Bee settings.
                    </span>
                  </div>
