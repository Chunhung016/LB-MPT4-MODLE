...
import { Sparkles, BookOpen, ArrowRight, ArrowLeft, Star, Volume2, VolumeX, Settings, GraduationCap } from 'lucide-react';
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-3 sm:p-4 relative max-w-3xl mx-auto w-full my-auto">
      ...
      <motion.div
        ...
        className="max-w-xl w-full bg-white/95 backdrop-blur-xl border-3 border-amber-300 rounded-3xl p-5 sm:p-7 shadow-[0_12px_36px_rgba(217,119,6,0.18)] text-center relative overflow-hidden"
      >
        ...
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-100/90 text-amber-900 border border-amber-300 text-xs sm:text-sm font-black uppercase tracking-wider mb-3 shadow-xs">
          ...
          <span>Bahasa Melayu SJKC/SJKT • Bahagian B</span>
        </div>

        {/* Title */}
        <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-amber-950 tracking-tight mb-1.5 leading-snug">
          Latihan Pemahaman Interaktif
        </h1>

        <p className="text-sm sm:text-base text-amber-850 font-bold mb-4 max-w-md mx-auto leading-relaxed">
          Petikan: {settings.passage.title || 'Sikap Baik Nirmala'}
        </p>

        {/* Info Grid */}
        <div className="grid grid-cols-2 gap-2.5 mb-4">
          <div className="bg-amber-50/80 border-2 border-amber-200 rounded-2xl p-2.5 sm:p-3 flex flex-col items-center">
            <span className="text-[10px] sm:text-xs font-bold text-amber-700 uppercase tracking-wide">Jumlah Soalan</span>
            <span className="text-lg sm:text-xl font-black text-amber-950">{totalQuestions} Soalan</span>
          </div>

          <div className="bg-amber-50/80 border-2 border-amber-200 rounded-2xl p-2.5 sm:p-3 flex flex-col items-center">
            <span className="text-[10px] sm:text-xs font-bold text-amber-700 uppercase tracking-wide">Jumlah Markah</span>
            <span className="text-lg sm:text-xl font-black text-amber-950">{totalMarks} Markah</span>
          </div>
        </div>
        ...
        <div className="bg-amber-100/60 border border-amber-300/80 rounded-2xl p-3 text-left mb-4 sm:mb-5 text-xs sm:text-sm text-amber-900 flex items-start gap-2.5">
          ...
          <div>
            <p className="font-bold text-amber-950 mb-0.5">Siri Pembelajaran Pintar AI:</p>
            <p className="text-amber-850 leading-snug">
              Baca petikan, lengkapkan jawapan Soalan 9 (a - e), dan jawab Soalan 10 (Pendapat) di bawah 30 perkataan yang akan **dinilai secara automatik menggunakan AI**! Tekan <b>Lebah 🐝</b> untuk klu beranimasi!
            </p>
          </div>
        </div>
